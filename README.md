...
- **查看任务后蓝点立即清除**：打开带完成提醒的任务后，会同步向电脑端确认已读并刷新任务列表，避免蓝点一直保留
- **失败状态判断更准确**：加强异常终态、审批卡片和不完整回复识别，减少失败任务被显示成正常完成
- **任务操作继续适配新版 GPT**：保留 5.5.3 的新任务首发路由、重命名、归档和确认流程修复
- **会员与刷新逻辑保持一致**：继续以服务器会员记录为权威，并维持统一的页面刷新链路
